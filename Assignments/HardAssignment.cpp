...
class MagicItem {
public:
    MagicItem(const std::string& name, const std::string & powerType,
              int charges):
              
                name_{name}, powerType_{powerType}, charges_{charges} {
                std::cout << name_ << " (" << powerType_ 
                          << " power) created with " << charges_ 
                          << " charges!\n";
    }
    ...
    void use() {
        if (charges_ > 0) {
            charges_--;
            std::cout << name_ << " (" << powerType_ 
                      << " power) used! Charges left: " << charges_ << "\n";
        } else {
            std::cout << name_ << " (" << powerType_ 
                      << " power) has no charges left!\n";
        }
    }
